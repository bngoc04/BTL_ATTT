...
def handle_init_chat(data):
    ...
    # Save session
    save_session(sender, receiver, {
        "aes_key": base64.b64encode(aes_key).decode(),
        "iv": None
    })

    # Send encrypted AES key and signature to receiver
...
def handle_send_message(data):
    ...
    if not session_data:
        emit("chat_error", {"message": "Session not established"})
        return
    ...
    # Send to receiver
    emit("receive_message", msg_package, room=f"user_{receiver}")
